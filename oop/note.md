...
- 继承变量函数的查找顺序问题    
    - 优先查找自己的变量
    - 没有则查找父类
    - 构造函数如果没有定义，则自动查找调用父类的构造函数
    - 如果本类有定义，则不再继续向上查找
- 构造函数
    - 是一类特殊的函数，在类进行实例化之前进行调用
    - 如果定义了构函数，则实例化时使用构造函数，不查找父类构造函数    
    - 如果没有定义，则自动查找父类构造函数
...
- 我们使用多继承的语法来实现MInxin
- 使用Minxin实现多继承时候要非常小心
    - 首先他必须表示某一单一功能，而不是某个物品
    - 职责必须单一，如果由多个功能，则写多个Minxim
    - Mixin不能依赖于子类的实现
    - 子类及时某一继承这个Mixin类，也照样工作，只是缺少某个功能
- 优点
...
- 无论哪种修饰符都是为了对成员属性进行相应的控制
    ...
    - 属性修饰符：适用当前类里面使用，控制一个类中的一个属性
# 6.类的内置属性

        __dict__:以字典的方式显示类的长远组成
        __doc__:获取类的文档信息
...
- 抽象类的使用
    ...
    - 抽象类的主要作用是设定类的标准，以便于开发的时候是具有统一的规范
